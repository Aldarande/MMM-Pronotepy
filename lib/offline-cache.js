...
/* ── Durées ───────────────────────────────────────────────────────────
 * Distinct de parseInterval (lib/format.js), qui retombe sur 60 min pour
...
function save (dir, key, data, now, refreshAfterMs) {
   ...
   * une charge utile rigoureusement identique — ce qui est le cas tous les
   * week-ends et pendant les vacances, soit une bonne part de l'année —
   * est de l'usure sans contrepartie.
   *
   * À l'intervalle par défaut (60 min), l'économie est modeste : le seuil
   * de rafraîchissement ci-dessous vaut une heure, donc la plupart des
   * collectes réécrivent de toute façon. Elle compte surtout pour les
   * configurations à intervalle court, où le même contenu serait sinon
   * réécrit plusieurs fois par heure.
   *
