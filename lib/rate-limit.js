...
/* =====================================================================
   ...
   DEUX GRANULARITÉS, et c'est essentiel :

     • Le PLANCHER entre deux tentatives est par ENFANT (compte + enfant).
       Il existe pour écraser les rafales d'une MÊME collecte. L'indexer
       par compte affamait le second enfant d'un compte parent : les deux
       instances démarrent à quelques millisecondes d'écart, la première
       passait, la seconde était bloquée — et la course se rejouait à
       l'identique au cycle suivant. Constaté le 2026-09-13 : 0 collecte
       sur 4 cycles pour le second enfant.

     • La SUSPENSION, le RECUL après échec et le PLAFOND QUOTIDIEN sont
       par COMPTE. C'est le compte que PRONOTE voit, et une sanction ou un
       refus concerne tout le compte, pas un enfant en particulier.

   Conséquence assumée : un rechargement de page coûte une
   authentification PAR ENFANT affiché, et non une seule. C'est borné par
   le nombre d'instances — deux ou trois — là où le défaut d'origine
...
function emptyState () {
  return {
    /* Dernière tentative par enfant : « compte-enfant » → horodatage.
     * C'est le plancher, la seule règle de granularité fine. */
    attempts:            {},
    /* Dernière tentative tous enfants confondus — sert au recul après
     * échec, qui est une affaire de compte. */
    lastAttempt:         0,
...
/* Identifiant du « créneau » : un enfant d'un compte, ou le compte seul
 * quand il n'y a pas d'enfant à choisir (compte élève). */
function slotKey (slot) {
  const texte = String(slot === undefined || slot === null ? '' : slot).trim();
  return texte || '_';
}

/* Dernière tentative pour ce créneau.
 *
 * Repli sur `lastAttempt` quand la carte est vide : c'est le cas d'un
 * état écrit par la 1.1.0, qui ignorait la granularité par enfant. Sans
 * ce repli, la mise à jour rouvrirait grand le plancher au premier
 * démarrage — exactement au moment où l'on ne veut pas de rafale. */
function lastAttemptFor (state, cle) {
  const carte = state.attempts || {};
  if (Object.prototype.hasOwnProperty.call(carte, cle)) return carte[cle];
  return Object.keys(carte).length === 0 ? (state.lastAttempt || 0) : 0;
}
 ...
 */
function evaluate (state, options, now, slot) {
  const s    = Object.assign(emptyState(), state || {});
  const opts = withDefaults(options);
  const t    = now === undefined ? Date.now() : now;
  const cle  = slotKey(slot);
  ...
  /* Plancher PAR ENFANT. L'indexer sur le compte affamerait le second
   * enfant d'un compte parent — voir l'en-tête du fichier. */
  const precedente = lastAttemptFor(s, cle);
  const depuis     = t - precedente;
  if (precedente > 0 && depuis < opts.minIntervalMs) {
    return {
...
function afterAttempt (state, now, slot) {
  const s    = Object.assign(emptyState(), state || {});
  const t    = now === undefined ? Date.now() : now;
  const jour = dayKey(t);

  /* La carte est bornée : sans cela, un `childName` qui change à chaque
   * démarrage la ferait grossir indéfiniment. Au-delà, on repart à vide
   * — perdre l'historique d'enfants disparus est sans conséquence. */
  const attempts = Object.assign({}, s.attempts);
  if (Object.keys(attempts).length > 32) {
    for (const k of Object.keys(attempts)) delete attempts[k];
  }
  attempts[slotKey(slot)] = t;
...
/**
 * État après une action manuelle de l'utilisateur — un scan de QR Code.
 *
 * On rescanne un QR Code PRÉCISÉMENT parce que les collectes échouaient :
 * un recul est donc presque toujours accumulé, et la collecte qui suit le
 * scan serait bloquée jusqu'à six heures. L'utilisateur verrait un écran
 * inchangé après avoir fait exactement ce qu'on lui demandait.
 *
 * On efface donc le recul et les planchers. Mais PAS `suspendedUntil` :
 * un nouveau jeton ne lève pas une sanction qui porte sur l'adresse IP,
 * et insister pendant une suspension la prolonge. Ni le compteur du jour,
 * qui doit rester un plafond honnête.
 */
function afterManualSetup (state) {
  const s = Object.assign(emptyState(), state || {});
  return Object.assign(s, {
    attempts:            {},
    lastAttempt:         0,
...
module.exports = {
  evaluate, afterAttempt, afterOutcome, afterManualSetup,
  emptyState, dayKey, backoffMs, humanize,
  slotKey, lastAttemptFor,
  load, save, stateFile,
