...
/* ── URL sûre pour les journaux ───────────────────────────────────────
 * La clé peut voyager en `?key=…` — c'est le seul canal disponible quand
 * on tape l'URL dans un navigateur. Or les refus journalisent l'URL, et
 * ces traces partent à la fois dans `console.warn` — donc le journal
 * système — et dans le tampon exposé par /api/logs.
 *
 * Le cas n'est pas théorique : un signet contenant la BONNE clé, utilisé
 * pendant la fenêtre de démarrage où la configuration n'est pas encore
 * parvenue au backend, est refusé par le repli loopback — et la clé
 * correcte se retrouve écrite en clair. */
  ...
  const middleware = (req, res, next) => {
    ...
    if (secret.length >= MIN_SECRET_LENGTH) {
      ...
      warn('Accès refusé (clé absente ou invalide) — ' + (req.method || '?') + ' ' +
           safeUrlForLog(req.originalUrl || req.url) + ' depuis ' + addr);
    ...
    warn('Accès refusé (aucune clé d\'API configurée, appel non local) — ' +
         (req.method || '?') + ' ' + safeUrlForLog(req.originalUrl || req.url) +
         ' depuis ' + addr);
    return deny(req, res, moduleName);
  };
