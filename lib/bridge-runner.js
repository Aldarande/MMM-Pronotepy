 ...
 */
/* Environnement du pont Python.
 *
 * Le pont a besoin de PATH, HOME, LANG… mais d'aucun secret du module.
 * Transmettre `process.env` en bloc lui donnait la clé d'API et le
 * réglage d'ouverture, qu'il n'utilise pas — et qu'une trace ou un vidage
 * mémoire pourrait exposer. Moindre privilège : on les retire. */
function bridgeEnv () {
  const env = { ...process.env, PYTHONIOENCODING: 'utf-8', PYTHONUNBUFFERED: '1' };
  for (const cle of Object.keys(env)) {
    if (/^MMM_PRONOTEPY_(API_KEY|ALLOW_UNAUTHENTICATED)$/.test(cle)) delete env[cle];
  }
  return env;
}
...
module.exports = { bridgeEnv, runBridge, DEFAULT_TIMEOUT_MS, DEFAULT_MAX_STDOUT, KILL_GRACE_MS };
