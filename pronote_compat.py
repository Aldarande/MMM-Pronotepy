...
#
# Fichier partagé avec le plugin Jeedom ProJote, même auteur :
#   https://github.com/Aldarande/ProJote — resources/ProJoted/pronote_compat.py
# ProJote est sous AGPL-3.0 ; cette copie-ci est distribuée sous MIT. Rien ne
# s'y oppose — l'auteur est le même et peut licencier son travail deux fois —
# mais les deux copies ne sont plus interchangeables : du code repris ICI ne
# peut pas repartir vers ProJote sous MIT sans y créer une incohérence.
...
entièrement puis rejoue la requête — qui échoue de nouveau, forcément, puisqu'une
nouvelle session ne donne aucun droit supplémentaire. Chaque authentification
fait tourner le jeton de connexion, et leur accumulation a valu une suspension
d'adresse IP par Pronote. Le correctif lève donc l'erreur avant d'entrer dans ce
mécanisme.

# Conditions de retrait

Sans date de péremption écrite, un correctif de compatibilité devient du code
mort que plus personne n'ose enlever. Voici donc, pour chacun, ce qui doit être
vrai pour le supprimer — les deux sont indépendants, l'un peut partir sans
l'autre.

| Correctif                  | Retirer quand…                                        |
|----------------------------|-------------------------------------------------------|
| Challenge non chiffré      | pronotepy intègre le correctif de l'issue #346         |
| Ré-authentification inutile| ``ClientBase.post`` ne réessaie plus sur un onglet     |
|                            | absent de ``authorized_onglets``                       |

Aucune version de pronotepy ne les rend inutiles à ce jour (dernière
vérification : 2.15.7, publiée le 2026-09-03 — les deux correctifs restent
nécessaires). Le premier des deux a une issue amont ouverte, le second n'en a
pas ; ce fichier est donc à revoir à chaque montée de version.

Procédure de vérification, à refaire à chaque montée de version — sur la version
candidate, dans le venv du module :

    # 1. Challenge : le correctif amont supprime l'étape « déchiffrer puis
    #    retirer l'aléa » pour le challenge. S'il est intégré, _login ne
    #    passe plus par aes_decrypt sur un bloc unique.
    python -c "import inspect, pronotepy.clients as c; print(inspect.getsource(c.ClientBase._login))" | grep -nE "alea|aes_decrypt"

    # 2. Onglets : le correctif amont consiste à ne pas réessayer. S'il est
    #    intégré, post() teste authorized_onglets avant sa boucle de retry.
    python -c "import inspect, pronotepy.clients as c; print(inspect.getsource(c.ClientBase.post))" | grep -n "authorized_onglets"

Puis, correctif retiré, une connexion réelle doit aboutir et les compteurs de
``fetch`` rester identiques. Relever alors le plancher de requirements.txt à la
première version corrigée : c'est ce plancher, et non ce fichier, qui garantit
que l'utilisateur a bien le correctif.
"""
...
# =====================================================================
# Prédicats purs — extraits pour être éprouvés sans pronotepy installé
# =====================================================================

def _challenge_brut(data):
    ...
    du déchiffrement ; on double donc chaque caractère pour qu'il retrouve la
    chaîne d'origine, qu'il rechiffrera et renverra au serveur.
    """
    return "".join(c * 2 for c in data.hex().upper()).encode()


def _est_challenge_clair(enc_self, chiffrement_session, data):
    """Ce bloc est-il le challenge d'un serveur PRONOTE >= 2026.2.5 ?

    Trois conditions, toutes nécessaires :

    - l'instance ``_Encryption`` appelée n'est pas celle de la communication.
      Celle-ci déchiffre les réponses du serveur et la clé de session dans
      ``after_auth`` ; seul le chiffrement local à ``_login`` traite le
      challenge ;
    - la taille fait exactement un bloc AES. C'est la signature du protocole
      2026 : les serveurs antérieurs renvoient une chaîne entrelacée d'aléa,
      toujours plus longue. Trancher sur la taille plutôt que sur l'échec du
      déchiffrement évite un piège — avec une mauvaise clé, le dépadding
      réussit par hasard environ une fois sur 256, et l'ancien chemin
      produirait alors une réponse fausse, soit un échec inexpliqué tous les
      deux jours environ au rythme du démon.
    """
    return (
...
def _onglet_refuse(autorises, onglet):
    """L'onglet demandé est-il hors des droits connus de la session ?

    ``autorises`` vide ou absent signifie « liste inconnue » et non « aucun
    droit » : on laisse alors passer, c'est pronotepy qui tranchera.
    """
    return bool(onglet is not None and autorises and onglet not in autorises)
...
def _install() -> None:
    ...
    def _login_avec_repli(self) -> bool:
        ...
        def _decrypt(enc_self, data: bytes) -> bytes:
            # Le critère complet — et sa justification — vit dans
            # _est_challenge_clair, au niveau du module : c'est le seul endroit
            # où la décision se prend, et il est éprouvé par les tests.
            if _est_challenge_clair(enc_self, chiffrement_session, data):
                logging.info(
    ...
    def _post_sans_reauth_inutile(self, function_name, onglet=None, data=None):
        ...
        if _onglet_refuse(autorises, onglet):
